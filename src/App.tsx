...
function ProbabilityCurve({ pct, color }: { pct: number; color: string }) {
  // A fun, interactive SVG curve representing probability
      ...
      >
        ...
        <div className="relative z-10">
          <div className="flex items-end justify-around h-48 mt-8 gap-4 border-b border-[#222] pb-0">
            {tgeList.map((item, i) => {
              const heightPct = Math.max(item.confidence || 0, 5); // min height
              
              // Format label to be shorter (e.g. "June 30, 2026" -> "Jun '26")
              let shortLabel = item.label;
              const dateMatch = item.label.match(/([A-Za-z]+)\s+\d{1,2},?\s+(\d{4})/);
              if (dateMatch) {
                shortLabel = `${dateMatch[1].slice(0, 3)} '${dateMatch[2].slice(2)}`;
              } else if (item.label.match(/202[5-9]/)) {
                shortLabel = item.label.replace(/20(2[5-9])/, "'$1");
              }
              ...
              return (
                <div key={i} className="flex flex-col items-center flex-1 h-full justify-end group">
                  <div className="w-full max-w-[60px] bg-[#1a1a1a] rounded-t-xl relative overflow-hidden flex items-end justify-center" style={{ height: '100%' }}>
                    ...
                    />
                    <span className="relative z-10 mb-2 text-xs font-black text-white drop-shadow-md">
                      {item.confidence}%
                    </span>
                  </div>
                  <div className="mt-3 text-[10px] text-gray-400 font-bold uppercase tracking-wider text-center h-8 flex items-start justify-center leading-tight">
                    {shortLabel}
                  </div>
                </div>
              );
          ...
          <div className="mt-6 text-center text-[10px] text-gray-500 uppercase tracking-widest font-bold">
            Top {tgeList.length} Estimated Dates
          </div>
        </div>
      </motion.div>
    </motion.div>
...
function FDVChartModal({ token, onClose }: { token: TokenData; onClose: () => void }) {
  // Get top 3 FDV markets by probability
  const topFDVs = [...(token.fdvMarkets || [])]
    .sort((a, b) => (b.yesPct || 0) - (a.yesPct || 0))
    .slice(0, 3);
    
  if (!topFDVs.length) return null;

  // Sort them by FDV value for the X-axis
    ...
    >
      ...
      >
        ...
        <div className="relative z-10 mt-8">
            ...
            {topFDVs.map((m, i) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="absolute bottom-full mb-2 text-white font-black text-sm drop-shadow-md">
                    {Math.round(m.yesPct || 0)}%
                  </div>
                  <div className="absolute top-full mt-4 text-gray-400 font-bold text-[10px] whitespace-nowrap">
                    {label}
                  </div>
                </motion.div>
              );
          ...
          <div className="mt-12 text-center text-[10px] text-gray-500 uppercase tracking-widest font-bold">
            Top {topFDVs.length} FDV Targets
          </div>
        </div>
      </motion.div>
    </motion.div>
...
function TokenCard({ token, onShowTGE, onShowFDV }: { key?: any; token: TokenData; onShowTGE: (t: TokenData) => void; onShowFDV: (t: TokenData) => void }) {
  ...
  // Find highest probability FDV market
  const highestFDV = token.fdvMarkets && token.fdvMarkets.length > 0 
    ? [...token.fdvMarkets].sort((a, b) => (b.yesPct || 0) - (a.yesPct || 0))[0]
    : null;
  ...
  return (
    <motion.div 
      ...
      className="bg-[#050505] border border-white/5 rounded-[2rem] p-6 transition-all duration-500 hover:border-white/10 hover:shadow-[0_8px_30px_rgba(255,255,255,0.04)] group relative overflow-hidden flex flex-col will-change-transform"
    >
      {/* Bento-style dotted background */}
      ...
      <div className="flex items-start gap-4 mb-6 relative z-10">
        <div className="w-12 h-12 rounded-2xl bg-white/5 border border-white/5 flex items-center justify-center text-lg font-black text-gray-500 overflow-hidden flex-shrink-0 shadow-inner group-hover:bg-white/10 group-hover:border-white/10 group-hover:text-white transition-all duration-500">
          {token.image ? (
            <img src={token.image} alt={token.title} className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-500" referrerPolicy="no-referrer" />
          ) : (
            token.title.slice(0, 2).toUpperCase()
          )}
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 text-[10px] text-gray-500 font-bold uppercase tracking-widest">
            <div className="flex items-center gap-1 bg-white/5 px-2 py-1 rounded-md backdrop-blur-sm group-hover:bg-white/10 transition-colors">
              <Activity className="w-3 h-3 text-[#00e5ff]" />
              <span>{formatCurrency(token.volume24hr)}</span>
            </div>
            <div className="flex items-center gap-1 bg-white/5 px-2 py-1 rounded-md backdrop-blur-sm group-hover:bg-white/10 transition-colors">
      ...
      <div className="flex-1 flex flex-col gap-4 relative z-10">
        {tge && (
          <button 
            onClick={() => onShowTGE(token)}
            className="w-full flex items-center gap-3 bg-white/5 border border-white/5 rounded-2xl px-4 py-3 hover:bg-white/10 hover:border-white/10 transition-all duration-300 group/tge"
          >
            <div className="w-8 h-8 rounded-lg bg-[#f59e0b]/10 flex items-center justify-center group-hover/tge:scale-110 group-hover/tge:bg-[#f59e0b]/20 transition-all">
              <Calendar className="w-4 h-4 text-[#f59e0b]" />
            </div>
            <div className="text-left">
              <div className="text-[9px] text-gray-500 font-black uppercase tracking-widest">Est. TGE</div>
              <div className="text-sm text-gray-200 font-bold">{tge.label}</div>
            </div>
        ...
        {highestFDV ? (
          <button 
            onClick={() => onShowFDV(token)}
            className="bg-white/5 rounded-2xl p-4 border border-white/5 hover:bg-white/[0.07] hover:border-white/10 transition-all duration-500 text-left w-full group/fdv"
          >
            <FDVDisplay market={highestFDV} />
          </button>
        ) : (
          <div className="flex-1 min-h-[80px] flex items-center justify-center border border-dashed border-white/10 rounded-2xl bg-white/5">
            <span className="text-[10px] text-gray-500 font-bold uppercase tracking-widest flex items-center gap-2">
              <Sparkles className="w-3 h-3" /> No FDV Data
            </span>
          </div>
        )}
      </div>
      ...
      <div className="mt-6 pt-4 border-t border-white/5 flex justify-between items-center relative z-10">
        <div className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">
          Liquidity <span className="text-gray-300 ml-1">{formatCurrency(token.liquidity)}</span>
        </div>
        <a 
          href={`https://polymarket.com/event/${token.slug}`}
          target="_blank"
          rel="noreferrer"
          className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center hover:bg-white hover:text-black transition-all duration-300 text-gray-400"
        >
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex flex-col gap-4 relative z-10">
        <div className="bg-white/5 rounded-2xl p-4 border border-white/5 group-hover:bg-white/[0.07] transition-colors duration-500">
          <div className="flex justify-between items-end mb-1">
            <div>
              <div className="text-[11px] text-gray-400 font-bold tracking-tight mb-1">Mark Price</div>
              <div className="text-xl font-black text-gray-100">${token.markPrice.toPrecision(4)}</div>
            </div>
            <div className="text-right">
              <div className="text-[11px] text-gray-400 font-bold tracking-tight mb-1">24h Change</div>
              <div className={`text-sm font-black ${isPositive ? 'text-[#00e5ff]' : 'text-[#f43f5e]'}`}>
                {isPositive ? '+' : ''}{token.priceChangePct.toFixed(2)}%
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-between items-center mt-3 pt-3 border-t border-white/5">
            <div>
              <div className="text-[9px] text-gray-500 font-black uppercase tracking-widest mb-1">Open Interest</div>
              <div className="text-xs font-bold text-gray-200">{formatCurrency(token.openInterest * token.markPrice)}</div>
            </div>
            <div className="text-right">
              <div className="text-[9px] text-gray-500 font-black uppercase tracking-widest mb-1">Funding Rate</div>
              <div className={`text-xs font-bold ${token.funding >= 0 ? 'text-[#f59e0b]' : 'text-[#00e5ff]'}`}>
                {token.funding >= 0 ? '+' : ''}{(token.funding * 100).toFixed(4)}%
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="mt-6 pt-4 border-t border-white/5 flex justify-between items-center relative z-10">
        ...
        <a 
          href={`https://app.hyperliquid.xyz/trade/${token.name}`}
          target="_blank"
          rel="noreferrer"
          className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center hover:bg-white hover:text-black transition-all duration-300 text-gray-400"
        >
...
function MexcCard({ token }: { token: MexcTokenData }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-4 mb-6 relative z-10">
        <div className="w-12 h-12 rounded-2xl bg-white/5 border border-white/5 flex items-center justify-center text-lg font-black text-gray-500 overflow-hidden flex-shrink-0 shadow-inner group-hover:bg-white/10 group-hover:border-white/10 group-hover:text-white transition-all duration-500">
          {token.vn.slice(0, 2).toUpperCase()}
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 text-[10px] text-gray-500 font-bold uppercase tracking-widest">
            <div className="flex items-center gap-1 bg-white/5 px-2 py-1 rounded-md backdrop-blur-sm group-hover:bg-white/10 transition-colors">
              <span className="text-[#10b981]">{token.vn}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col gap-4 relative z-10">
        <div className="bg-white/5 rounded-2xl p-4 border border-white/5 group-hover:bg-white/[0.07] transition-colors duration-500">
          <div className="flex justify-between items-end mb-3">
            <div>
              <div className="text-[11px] text-gray-400 font-bold tracking-tight mb-1">Last Price</div>
              <div className="text-xl font-black text-gray-100">${token.price.toPrecision(4)}</div>
            </div>
            <div className="text-right">
              <div className="text-[11px] text-gray-400 font-bold tracking-tight mb-1">24h Change</div>
              <div className={`text-sm font-black ${isPositive ? 'text-[#10b981]' : 'text-[#f43f5e]'}`}>
                {isPositive ? '+' : ''}{token.priceChangePct.toFixed(2)}%
              </div>
            </div>
          </div>
          <p className="text-xs text-gray-400 line-clamp-3 leading-relaxed border-t border-white/5 pt-3">
            {token.idu}
          </p>
        </div>
      </div>
      ...
      <div className="mt-6 pt-4 border-t border-white/5 flex justify-between items-center relative z-10">
        <div className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">
          Est. Volume <span className="text-gray-300 ml-1">{formatCurrency(token.volume)}</span>
        </div>
        <a 
          href={`https://www.mexc.com/exchange/${token.vn}_USDT`}
          target="_blank"
          rel="noreferrer"
          className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center hover:bg-white hover:text-black transition-all duration-300 text-gray-400"
        >
...
function InteractiveBackground() {
  // A fun, interactive background with floating elements that respond to mouse movement
  // Optimized using useMotionValue and useSpring to avoid React re-renders on mouse move
  ...
  // Create transforms for the different glowing orbs
  ...
  return (
    <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
      ...
      <motion.div 
        style={{ x: x1, y: y1 }}
        className="absolute top-[10%] left-[20%] w-[40rem] h-[40rem] bg-[#00e5ff]/[0.04] rounded-full blur-[120px]"
      />
      <motion.div 
        style={{ x: x2, y: y2 }}
        className="absolute bottom-[10%] right-[20%] w-[45rem] h-[45rem] bg-[#8b5cf6]/[0.04] rounded-full blur-[150px]"
      />
      <motion.div 
        style={{ x: x3, y: y3 }}
        className="absolute top-[40%] left-[50%] -translate-x-1/2 w-[50rem] h-[50rem] bg-[#f59e0b]/[0.03] rounded-full blur-[150px]"
      />
      
      {/* Grid pattern with radial fade */}
      <div 
        className="absolute inset-0 bg-[url('data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iNDAiIGhlaWdodD0iNDAiIHhtbG5zPSJodHRwOi8vd3d3LnczLm9yZy8yMDAwL3N2ZyI+PGNpcmNsZSBjeD0iMSIgY3k9IjEiIHI9IjEiIGZpbGw9InJnYmEoMjU1LDI1NSwyNTUsMC4wNSkiLz48L3N2Zz4=')] opacity-30" 
        style={{ 
...
const headerOpacity = useTransform(scrollYProgress, [0, 0.2], [1, 0]);
  ...
  const fetchData = async () => {
    try {
      ...
      setError(e.message || "Failed to fetch data. Make sure the backend is running.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchHlData = async () => {
    try {
      ...
      if (res.data.success) {
        setHlData(res.data.data);
      } else {
        throw new Error(res.data.error || "Failed to fetch HL data");
      }
    } catch (e: any) {
      console.error("HL Fetch error:", e);
      setHlError(e.message || "Failed to fetch Hyperliquid data.");
    } finally {
      setHlLoading(false);
    }
  };
  ...
  const fetchMexcData = async () => {
    try {
      ...
      if (res.data.success) {
        ...
        throw new Error(res.data.error || "Failed to fetch MEXC data");
      }
    } catch (e: any) {
      console.error("MEXC Fetch error:", e);
      setMexcError(e.message || "Failed to fetch MEXC data.");
    } finally {
  ...
  const filteredData = data.filter(token => 
    ...
    token.slug.toLowerCase().includes(searchQuery.toLowerCase())
  );

  const filteredHlData = hlData.filter(token => 
    token.name.toLowerCase().includes(searchQuery.toLowerCase())
  );

  const filteredMexcData = mexcData.filter(token => 
    token.vn.toLowerCase().includes(searchQuery.toLowerCase()) ||
    token.fn.toLowerCase().includes(searchQuery.toLowerCase())
  );
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 pb-16 md:pb-24">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-12">
          <div className="flex justify-center gap-4">
            <button 
              ...
              className={`px-6 py-3 rounded-full text-sm font-black uppercase tracking-widest transition-all ${activeTab === 'polymarket' ? 'bg-[#00e5ff] text-black shadow-[0_0_20px_rgba(0,229,255,0.4)]' : 'bg-[#111] text-gray-500 hover:text-white border border-[#222]'}`}
            >
              Polymarket
            </button>
            <button 
              onClick={() => setActiveTab('hyperliquid')}
              className={`px-6 py-3 rounded-full text-sm font-black uppercase tracking-widest transition-all ${activeTab === 'hyperliquid' ? 'bg-[#8b5cf6] text-black shadow-[0_0_20px_rgba(139,92,246,0.4)]' : 'bg-[#111] text-gray-500 hover:text-white border border-[#222]'}`}
            >
              Hyperliquid
            </button>
            <button 
              onClick={() => setActiveTab('mexc')}
              className={`px-6 py-3 rounded-full text-sm font-black uppercase tracking-widest transition-all ${activeTab === 'mexc' ? 'bg-[#10b981] text-black shadow-[0_0_20px_rgba(16,185,129,0.4)]' : 'bg-[#111] text-gray-500 hover:text-white border border-[#222]'}`}
            >
              MEXC
            </button>
          </div>
          ...
          <div className="relative w-full md:w-72">
            ...
            <input
              ...
              className="w-full bg-[#111]/50 border border-[#222] text-white text-sm rounded-full pl-10 pr-4 py-3 focus:outline-none focus:border-[#00e5ff]/50 focus:ring-1 focus:ring-[#00e5ff]/50 transition-all placeholder-gray-600"
            />
          </div>
        </div>
        ...
        {activeTab === 'polymarket' && (
          <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-12 pb-8 border-b border-[#111]">
            <div className="flex flex-wrap justify-center md:justify-start items-center gap-8">
              <div className="group">
                <div className="text-[10px] text-gray-600 font-black uppercase tracking-widest mb-1 group-hover:text-[#00e5ff] transition-colors">Tracked Assets</div>
                <div className="text-2xl font-black text-white">{filteredData.length}</div>
              </div>
              <div className="hidden md:block w-px h-8 bg-[#222]" />
              <div className="group">
                <div className="text-[10px] text-gray-600 font-black uppercase tracking-widest mb-1 group-hover:text-[#8b5cf6] transition-colors">Total Volume</div>
                <div className="text-2xl font-black text-white">{formatCurrency(filteredData.reduce((acc, curr) => acc + curr.volume, 0))}</div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-6">
              ...
              <button 
                onClick={fetchData}
                disabled={loading}
                className="group relative bg-white text-black px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest hover:scale-105 transition-all active:scale-95 disabled:opacity-50 shadow-[0_0_20px_rgba(255,255,255,0.2)]"
              >
        ...
        {activeTab === 'hyperliquid' && (
          <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-12 pb-8 border-b border-[#111]">
            <div className="flex flex-wrap justify-center md:justify-start items-center gap-8">
              <div className="group">
                <div className="text-[10px] text-gray-600 font-black uppercase tracking-widest mb-1 group-hover:text-[#8b5cf6] transition-colors">Tracked Perps</div>
                <div className="text-2xl font-black text-white">{hlData.length}</div>
              </div>
              <div className="hidden md:block w-px h-8 bg-[#222]" />
              <div className="group">
                <div className="text-[10px] text-gray-600 font-black uppercase tracking-widest mb-1 group-hover:text-[#8b5cf6] transition-colors">Total Volume</div>
                <div className="text-2xl font-black text-white">{formatCurrency(hlData.reduce((acc, curr) => acc + curr.volume24h, 0))}</div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-6">
              ...
              >
                <span className="relative z-10 flex items-center gap-2">
                  {hlLoading ? "Syncing..." : "Refresh"}
                  <RefreshCw className={`w-3 h-3 ${hlLoading ? 'animate-spin' : ''}`} />
                </span>
              </button>
            </div>
          </div>
        ...
        {activeTab === 'mexc' && (
          <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-12 pb-8 border-b border-[#111]">
            <div className="flex flex-wrap justify-center md:justify-start items-center gap-8">
              <div className="group">
                <div className="text-[10px] text-gray-600 font-black uppercase tracking-widest mb-1 group-hover:text-[#10b981] transition-colors">Tracked Coins</div>
                <div className="text-2xl font-black text-white">{mexcData.length}</div>
              </div>
              <div className="hidden md:block w-px h-8 bg-[#222]" />
              <div className="group">
                <div className="text-[10px] text-gray-600 font-black uppercase tracking-widest mb-1 group-hover:text-[#10b981] transition-colors">Total Volume</div>
                <div className="text-2xl font-black text-white">{formatCurrency(mexcData.reduce((acc, curr) => acc + curr.volume, 0))}</div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-6">
              ...
              <button 
                onClick={fetchMexcData}
                disabled={mexcLoading}
                className="group relative bg-white text-black px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest hover:scale-105 transition-all active:scale-95 disabled:opacity-50 shadow-[0_0_20px_rgba(255,255,255,0.2)]"
              >
                <span className="relative z-10 flex items-center gap-2">
                  {mexcLoading ? "Syncing..." : "Refresh"}
                  <RefreshCw className={`w-3 h-3 ${mexcLoading ? 'animate-spin' : ''}`} />
                </span>
              </button>
            </div>
          </div>
        ...
        <AnimatePresence>
          {(error || hlError || mexcError) && (
            <motion.div 
              ...
              className="bg-red-500/5 border border-red-500/20 rounded-2xl p-4 mb-8 flex items-center gap-3"
            >
        ...
        {activeTab === 'polymarket' && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {loading && data.length === 0 ? (
              [...Array(8)].map((_, i) => (
                <div key={i} className="h-[380px] bg-[#111]/50 border border-[#222] rounded-[2rem] animate-pulse" />
              ))
            ) : (
              <AnimatePresence mode="popLayout">
                {filteredData.map((token) => (
                  <TokenCard key={token.id} token={token} onShowTGE={setSelectedTGE} onShowFDV={setSelectedFDV} />
                ))}
              </AnimatePresence>
            )}
          </div>
        ...
        {activeTab === 'hyperliquid' && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {hlLoading && hlData.length === 0 ? (
              [...Array(8)].map((_, i) => (
                <div key={i} className="h-[380px] bg-[#111]/50 border border-[#222] rounded-[2rem] animate-pulse" />
              ))
            ) : (
              <AnimatePresence mode="popLayout">
                {filteredHlData.map((token) => (
                  <HyperliquidCard key={token.name} token={token} />
                ))}
              </AnimatePresence>
            )}
          </div>
        ...
        {activeTab === 'mexc' && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {mexcLoading && mexcData.length === 0 ? (
              [...Array(8)].map((_, i) => (
                <div key={i} className="h-[380px] bg-[#111]/50 border border-[#222] rounded-[2rem] animate-pulse" />
              ))
            ) : (
              <AnimatePresence mode="popLayout">
                {filteredMexcData.map((token) => (
                  <MexcCard key={token.id} token={token} />
                ))}
              </AnimatePresence>
            )}
          </div>
        ...
        {/* Empty State */}
        {activeTab === 'polymarket' && !loading && filteredData.length === 0 && !error && (
          <div className="text-center py-32 bg-[#111]/30 border border-[#222] rounded-[3rem] border-dashed backdrop-blur-sm">
            <BarChart3 className="w-12 h-12 text-gray-700 mx-auto mb-4" />
            <h3 className="text-gray-300 text-lg font-black tracking-tight">No active markets detected</h3>
            <p className="text-gray-500 text-sm mt-2">Try adjusting your search or check back later.</p>
          </div>
        )}
        
        {activeTab === 'hyperliquid' && !hlLoading && filteredHlData.length === 0 && !hlError && (
          <div className="text-center py-32 bg-[#111]/30 border border-[#222] rounded-[3rem] border-dashed backdrop-blur-sm">
            <BarChart3 className="w-12 h-12 text-gray-700 mx-auto mb-4" />
            <h3 className="text-gray-300 text-lg font-black tracking-tight">No active pre-launch perps detected</h3>
            <p className="text-gray-500 text-sm mt-2">Try adjusting your search or check back later.</p>
          </div>
        )}

        {activeTab === 'mexc' && !mexcLoading && filteredMexcData.length === 0 && !mexcError && (
          <div className="text-center py-32 bg-[#111]/30 border border-[#222] rounded-[3rem] border-dashed backdrop-blur-sm">
            <BarChart3 className="w-12 h-12 text-gray-700 mx-auto mb-4" />
            <h3 className="text-gray-300 text-lg font-black tracking-tight">No active MEXC premarket coins detected</h3>
            <p className="text-gray-500 text-sm mt-2">Try adjusting your search or check back later.</p>
          </div>
        )}
        ...
        <AnimatePresence>
          {selectedTGE && (
            <TGEChartModal token={selectedTGE} onClose={() => setSelectedTGE(null)} />
          )}
          {selectedFDV && (
            <FDVChartModal token={selectedFDV} onClose={() => setSelectedFDV(null)} />
          )}
        </AnimatePresence>
      </div>
    </div>
